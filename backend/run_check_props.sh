#!/usr/bin/env bash
# One-shot read-only inspector: list devices + their actuator_properties
# for the target home. Reuses the deploy harness from run_db_cleanup.sh.

set -euo pipefail

REGION="us-east-1"
TMP_FN="oneshot-check-props-$(date +%s)"
SOURCE_PY="backend/_check_props.py"
