...
def lambda_handler(event, context):
    try:
        # 1. API Gateway'den gelen path parametresinden homeID'yi alıyoruz (/prod/{homeID}/automations)
        path_parameters = event.get('pathParameters') or {}
        home_id = path_parameters.get('homeID')
        
        # 2. Query string'den aranacak duyguyu alıyoruz (?emotion=happy)
        query_string_parameters = event.get('queryStringParameters') or {}
        emotion = query_string_parameters.get('emotion')
        
        # 3. Parametrelerin eksik olup olmadığını kontrol ediyoruz
        ...
        # 4. Veritabanına bağlanıyoruz (Secret'lar veya environment variable üzerinden)
        ...
        for row in rows:
            dev_id = row['deviceid']
            # Eğer bu cihaz henüz listeye eklenmediyse, ilk yapısını kuruyoruz
            ...
            # Cihazın komut listesine yeni özelliği (property_name) ve değerini ekliyoruz
            actions_dict[dev_id]["commands"].append({
                "property_name": row['property_name'],
                "value": row['target_value']
            })
            
        # Sozluk yapısını listeye çeviriyoruz
        actions_list = list(actions_dict.values())
        
        # 8. API dökümanındaki formata birebir uyan final response objesini oluşturuyoruz
        response_body = {
            "homeID": home_id,
        ...
        # 9. Veritabanı bağlantılarını kapatıyoruz
        cur.close()
        conn.close()
        
        # 10. Sonucu JSON olarak API Gateway'e döndürüyoruz
        return {
            'statusCode': 200,
            'headers': {
                'Content-Type': 'application/json',
        ...
        # Ciddi hatalarda client'a 500 kodu ile bildiriyoruz
        return {
            'statusCode': 500,
            'headers': {
                'Content-Type': 'application/json',
